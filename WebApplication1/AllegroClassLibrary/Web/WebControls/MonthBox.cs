...
//
// 1.0.0 2004/04/01
// 管理番号B17056 2006/01/27 日付を1ヶ月前後させるボタン押下にてダーティONしないオプション追加
// 1.5.0 2006/03/31
...
namespace Infocom.Allegro.Web.WebControls
{
	...
	/// </summary>
	[
		ToolboxData("<{0}:MonthBox runat=server></{0}:MonthBox>")
	]
	public class MonthBox : System.Web.UI.WebControls.TextBox, IAllegroContorl
// 管理番号K26528 From
		, IBlurHandleable
...
//		/// <summary>
//		/// 年月入力用カスタムコントロールのコンストラクタです。
//		/// </summary>
//		public MonthBox()
//		{
//			base.Width = Unit.Pixel(51);
//		}
//		#endregion
// 管理番号K26528 To
		...
		/// <summary>
		/// テキストボックスに手動で入力できる最大文字数。常に7を返します。
		/// </summary>
		[
		Browsable(false),
		EditorBrowsable(EditorBrowsableState.Never)
		...
		public virtual DateTime MaxValue
		{
			...
			set {ViewState["MaxValue"] = new DateTime(value.Year, value.Month, 1);}
		}
		...
		public virtual DateTime MinValue
		{
			...
			set {ViewState["MinValue"] = new DateTime(value.Year, value.Month, 1);}
		}
...
// 管理番号B17056 From
		...
		Category("Behavior"),
		DefaultValue(true),
		Description("日付を1ヶ月前後させるボタン押下時のダーティON要否です。"),
		]
...
// 管理番号B17056 To
		...
		#region Methods
		/// <summary>
		/// MonthBox.Textを<see cref="DateTime"/>として取得します。
		/// </summary>
		/// <returns>
		/// MonthBox.Textを<see cref="DateTime"/>に変換した値。
		/// </returns>
		/// <remarks>
		/// MonthBox.Textが空の文字列の場合、例外が発生します。
		/// </remarks>
...
// 管理番号K26528 To
		...
		protected virtual string GetStartUpScript()
		{
			...
			return "registerBoxKeyDownEvent(\"" + ClientID + "\", addMonthBoxOnKeyDown);";
		}
// 管理番号K25901 To
		#endregion
...
//					script.Append("if(").Append(ClientChangeScript);
////管理番号 B24094 From
////					script.Append(IsRequiredField ? "(this)&&validateMonth(this,true)&&(!mouseOnButton||pressReturnKey)){allowPostBack=false;" : "(this)&&validateMonth(this,false)&&(!mouseOnButton||pressReturnKey)){allowPostBack=false;");
//					script.Append(IsRequiredField ? "(this)&&validateMonth(this,true)&&(!mouseOnButton||pressReturnKey)){" : "(this)&&validateMonth(this,false)&&(!mouseOnButton||pressReturnKey)){");
////管理番号 B24094 To
//					script.Append(Page.ClientScript.GetPostBackEventReference(this, string.Empty));
//					script.Append(";}");
//				}
...
//					script.Append(IsRequiredField ? "(this);validateMonth(this,true);" : "(this);validateMonth(this,false);");
//				}
////管理番号 K25901 From
////				writer.AddAttribute(HtmlTextWriterAttribute.Onchange, script.ToString());
////管理番号 K25901 To
...
//////管理番号 B24094 From
////////管理番号K20525 From
////////				StringBuilder script = new StringBuilder(IsRequiredField ? "textChanged=true;if(validateMonth(this,true)&&(!mouseOnButton||pressReturnKey)){allowPostBack=false;" : "textChanged=true;if(validateMonth(this,false)&&(!mouseOnButton||pressReturnKey)){allowPostBack=false;", 128);
//////				StringBuilder script = new StringBuilder(IsRequiredField ? "textChanged=true;validateChecked=true;if(validateMonth(this,true)&&(!mouseOnButton||pressReturnKey)){allowPostBack=false;" : "textChanged=true;validateChecked=true;if(validateMonth(this,false)&&(!mouseOnButton||pressReturnKey)){allowPostBack=false;", 128);
////////管理番号K20525 To
////				StringBuilder script = new StringBuilder(IsRequiredField ? "textChanged=true;validateChecked=true;if(validateMonth(this,true)&&(!mouseOnButton||pressReturnKey)){" : "textChanged=true;validateChecked=true;if(validateMonth(this,false)&&(!mouseOnButton||pressReturnKey)){", 128);
//////管理番号 B24094 To
//				script.Append(IsRequiredField ? "textChanged=true;validateChecked=true;if(validateMonth(this,true)&&(!mouseOnButton||pressReturnKey)){" : "textChanged=true;validateChecked=true;if(validateMonth(this,false)&&(!mouseOnButton||pressReturnKey)){");
////管理番号 K25901 To
//				script.Append(Page.ClientScript.GetPostBackEventReference(this, string.Empty));
//				script.Append(";}");
////管理番号 K25901 From
...
////管理番号 K25901 From
//////管理番号K20525 From
//////				writer.AddAttribute(HtmlTextWriterAttribute.Onchange, IsRequiredField ? "validateMonth(this,true)" : "validateMonth(this,false)");
////				writer.AddAttribute(HtmlTextWriterAttribute.Onchange, IsRequiredField ? "validateChecked=true;validateMonth(this,true)" : "validateChecked=true;validateMonth(this,false)");
//////管理番号K20525 To
//				script.Append(IsRequiredField ? "validateChecked=true;validateMonth(this,true)" : "validateChecked=true;validateMonth(this,false)");
////管理番号 K25901 To
//			}
//#endif
////管理番号 K25901 From
...
//			script2.Append(IsRequiredField ? "validateMonthOnBlur(this,true);" : "validateMonthOnBlur(this,false);");
//			if(ClientBlurScript.Length != 0)
//			{
...
// 				prev.ToolTip = "年月を 1 月戻します。"; //K24546
				prev.ToolTip = MultiLanguage.Get("CM_AM001328");
// 				next.ToolTip = "年月を 1 月進めます。"; //K24546
				next.ToolTip = MultiLanguage.Get("CM_AM001327");
// 管理番号 K24546 From
...
// 管理番号K26528 From
//// 管理番号B17056 From
////				prev.ClientClickScript2 = "setPreviousMonth(getElementById('" + ClientID + (AutoPostBack ? "'),this,true)" : "'),this,false)");
////				next.ClientClickScript2 = "setNextMonth(getElementById('" + ClientID + (AutoPostBack ? "'),this,true)" : "'),this,false)");
//				prev.ClientClickScript2 = "setPreviousMonth(getElementById('" + ClientID + (AutoPostBack ? "'),this,true," : "'),this,false,") + (PrevNextButtonDirty ? "true)" : "false)");
//				next.ClientClickScript2 = "setNextMonth(getElementById('" + ClientID + (AutoPostBack ? "'),this,true," : "'),this,false,") + (PrevNextButtonDirty ? "true)" : "false)");
//// 管理番号B17056 To
				...
				prev.ClientClickScript2 = "setPreviousMonth(" + arguments + ")";
				next.ClientClickScript2 = "setNextMonth(" + arguments + ")";
