...
// Module      : --
// Function    : --
// File Name   : CustomLinkButton.cs
// 機能名      : リンクボタンカスタムコントロール
// Version     : 3.0.0
// Last Update : 2018/04/30
// Copyright (c) 2004-2018 Grandit Corp. All Rights Reserved.
//
// 1.0.0 2004/04/01
// 管理番号 B12850 2004/09/17 Enable を false にした時、クリックイベントが走ってしまう不具合の修正
...
// 管理番号 B24414 2012/04/12 hrefに関数呼び出しが指定されていると、onclick内のsubmitが無効化されるIE6不具合対応
// 管理番号 B24449 2012/08/06 IE6の不具合回避の為、親要素にクリックイベントが実行されないように修正
// 2.0.0 2012/10/31
// 2.2.0 2014/10/31
...
namespace Infocom.Allegro.Web.WebControls
{
	...
	/// </summary>
	/// <remarks>
	/// <b>CustomLinkButton</b> は GRANDIT用に Enter キーによるフォーカス移動、ファンクションキーの割当等をサポートします。
	/// </remarks>
	[
	DefaultProperty("Text"),
	ToolboxData("<{0}:CustomLinkButton runat=server>CustomLinkButton</{0}:CustomLinkButton>")
	]
	public class CustomLinkButton : System.Web.UI.WebControls.LinkButton, IAllegroContorl
	{
		#region Properties
		/// <summary>
		...
		/// <para>
		/// <see cref="ClientClickScript"/>と<see cref="ClientClickScript2"/>が両方設定された場合、
		/// <see cref="ClientClickScript"/>のみが有効となります。
		/// </para>
		...
		Category("Behavior"),
		DefaultValue(""),
		Description("リンクボタンがクリックされた時に動作するクライアント スクリプトを指定します。")
		]
		...
		/// <summary>
		/// コントロールがクリックされた時に動作するクライアントスクリプト。
		/// クライアントスクリプトにはメソッドの呼び出し記述を設定してください。
		/// </summary>
		/// <remarks>
		/// <see cref="ClientClickScript"/>と<see cref="ClientClickScript2"/>が両方設定された場合、
		/// <see cref="ClientClickScript"/>のみが有効となります。
		...
		Category("Behavior"),
		DefaultValue(""),
		Description("イメージ ボタンがクリックされた時に動作するクライアント スクリプトを指定します。")
		]
		public virtual string ClientClickScript2
		{
			get
			{
				object o = ViewState["ClientClickScript2"];
				return (o != null) ? (string) o : string.Empty;
			}
			set {ViewState["ClientClickScript2"] = value;}
		}
...
////			{
////				StringBuilder script = new StringBuilder("if(allowPostBack && (", 128).Append(ClientClickScript).Append("(this)!=false) && forbidDuplicationSubmit())return true;else return false;");
////				writer.AddAttribute(HtmlTextWriterAttribute.Onclick, script.ToString());
////			}
////			else if (ClientClickScript2.Length != 0)
////			{
////				StringBuilder script = new StringBuilder("if(allowPostBack && (", 128).Append(ClientClickScript2).Append("!=false) && forbidDuplicationSubmit())return true;else return false;");
////				writer.AddAttribute(HtmlTextWriterAttribute.Onclick, script.ToString());
////			}
////			else
////			{
////				writer.AddAttribute(HtmlTextWriterAttribute.Onclick, "return forbidDuplicationSubmit();");
////			}
//
//			if (Enabled)
//			{
...
//////// 管理番号 B24094 From
////////					StringBuilder script = new StringBuilder("if(allowPostBack && (", 128).Append(ClientClickScript).Append("(this)!=false) && forbidDuplicationSubmit())return true;else return false;");
//////					StringBuilder script = new StringBuilder("if(checkSubmit() && (", 128).Append(ClientClickScript).Append("(this)!=false))return true;else return false;");
//////// 管理番号 B24094 To
////					StringBuilder script = new StringBuilder("if(checkSubmit() && (", 128).Append(ClientClickScript).Append("(this)!=false) && checkSubmit())return true;else return false;");
////// 管理番号 B24414 To
//					StringBuilder script = new StringBuilder("event.cancelBubble=true;if(checkSubmit() && (", 128).Append(ClientClickScript).Append("(this)!=false) && checkSubmit())return true;else return false;");
//// 管理番号 B24449 To
//					writer.AddAttribute(HtmlTextWriterAttribute.Onclick, script.ToString());
//				}
//				else if (ClientClickScript2.Length != 0)
...
//////// 管理番号 B24094 From
////////					StringBuilder script = new StringBuilder("if(allowPostBack && (", 128).Append(ClientClickScript2).Append("!=false) && forbidDuplicationSubmit())return true;else return false;");
//////					StringBuilder script = new StringBuilder("if(checkSubmit() && (", 128).Append(ClientClickScript2).Append("!=false))return true;else return false;");
//////// 管理番号 B24094 To
////					StringBuilder script = new StringBuilder("if(checkSubmit() && (", 128).Append(ClientClickScript2).Append("!=false) && checkSubmit())return true;else return false;");
////// 管理番号 B24414 To
//					StringBuilder script = new StringBuilder("event.cancelBubble=true;if(checkSubmit() && (", 128).Append(ClientClickScript2).Append("!=false) && checkSubmit())return true;else return false;");
//// 管理番号 B24449 To
//					writer.AddAttribute(HtmlTextWriterAttribute.Onclick, script.ToString());
//				}
//// 管理番号 B24094 From
...
//			writer.AddAttribute("onmouseover", "mouseOnButton=true");
//			writer.AddAttribute("onmouseout", "mouseOnButton=false");
			...
			{
				var scriptReturnValue = "true";
				if (!string.IsNullOrEmpty(ClientClickScript))
				{
					scriptReturnValue = ClientClickScript + "(this)";
				}
				else if (!string.IsNullOrEmpty(ClientClickScript2))
				{
					scriptReturnValue = ClientClickScript2;
				}

				var script = "return __customControl.handler.linkButtonClickHandler(this, function() { return " + scriptReturnValue + "; });";
...
//			if (Controls.Count == 1 && Controls[0] is DataBoundLiteralControl)
//			{
//				writer.Write(HttpUtility.HtmlEncode(((DataBoundLiteralControl) Controls[0]).Text.Trim()));
//			}
//			else
//			{
